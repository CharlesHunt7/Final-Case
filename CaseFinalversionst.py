...
# Assume you have defined PaxNLmonths and other necessary data
...
for airport, color in colors.items():
    ...
    airports.append(airport)
    residuals_data.append(df['Totaal Vluchten'] - np.exp(intercept + slope * x))
    ss_res = np.sum(residuals_data[-1]**2)
    ss_tot = np.sum((df['Totaal Vluchten'] - np.mean(df['Totaal Vluchten']))**2)
    r2_data.append(1 - (ss_res / ss_tot))
...
st.title('Kaart met airport data')
